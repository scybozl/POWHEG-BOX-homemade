...
C     -- WpWp2j stuff 
      ...
      data ini/.true./
      data debug/.true./
      real * 8 smincuts,powheginput,smin
      external smincuts,powheginput
      save ini,smin
      ...
      if(ini) then
         ...
         smin=powheginput("#smincuts")
         if(smin.lt.0) smin=0.1
         ini=.false.
      endif
C     
      xmin  = 1d-8 
...
C     mass3, width3 not used for n3 = 0 
      ...
      call phi1_2(xborn(1),xborn(2),tmp,xborn(4),p12,p3456,p78,
     .     n2,n3,mass2,mass3,width2,width3,wt12)
      ...
      call phi1_2(xborn(5),xborn(6),tmp,xborn(8),p3456,p56,p34,
     .     n2,n3,mass2,mass3,width2,width3,wt3456)

      call phi3m0(xborn(13),xborn(14),p34,p3,p4,wt34)
      call phi3m0(xborn(11),xborn(12),p56,p5,p6,wt56)
      call phi3m0(xborn(15),xborn(16),p78,p7,p8,wt78)
...
c     -- checks invariants, mom. conservation etc in Lab frame  
     ...
     .        kn_pborn(3,i)*kn_pborn(3,i)
      ...
      write(*,*) '----Transverse momenta----'
      do i=1,8
         write(*,*) 'p', i, ': ',sqrt(kn_pborn(1,i)**2+kn_pborn(2,i)**2)
      enddo

      write(*,*) 'xjac', kn_jacborn
...
c     -- checks invariants, mom. conservation etc in CM frame  
     ...
     .        kn_cmpborn(3,i)*kn_cmpborn(3,i)
      ...
      do i=1,8
         write(*,*) 'p',i, ': ',
     .         sqrt(kn_cmpborn(1,i)**2+kn_cmpborn(2,i)**2)
      enddo
...
c         write(*,*) '    scales (mur, muf) set to ' 
c         write(*,*) '    sum_{i=1,2}(ptj_i+sqrt(MW^2+ptw_i^2))/2'
C         write(*,*) '    scales set to the inv. mass of the W  '
         write(*,*) '    scales set to m_t / 2            '
         write(*,*) '*************************************'
C         fixedscale=powheginput("#fixedscale")
         ini=.false.
      endif
...
c      ptj1 = sqrt(kn_cmpborn(1,7)**2+kn_cmpborn(2,7)**2)                  ! pt_j1
c      ptj2 = sqrt(kn_cmpborn(1,8)**2+kn_cmpborn(2,8)**2)                  ! pt_j2 
c      etw1 = sqrt(ph_wmass**2+(kn_cmpborn(1,3)+kn_cmpborn(1,4))**2        ! ET_W1
c     .     +                  (kn_cmpborn(2,3)+kn_cmpborn(2,4))**2)
c      etw2 = sqrt(ph_wmass**2+(kn_cmpborn(1,5)+kn_cmpborn(1,6))**2        ! ET_W2
c     .     +                  (kn_cmpborn(2,5)+kn_cmpborn(2,6))**2)

c      muref = (ptj1+ptj2+etw1+etw2)/2d0 
!      muref = sqrt((ptj1**2+ptj2**2+etw1**2+etw2**2)/2d0) 
      muref = 86
     ...
     3     sqrt((kn_cmpborn(0,7)+kn_cmpborn(0,8))**2
     4         -(kn_cmpborn(1,7)+kn_cmpborn(1,8))**2
     5         -(kn_cmpborn(2,7)+kn_cmpborn(2,8))**2
     6         -(kn_cmpborn(3,7)+kn_cmpborn(3,8))**2))
      end
